...
from flask_datatables.views.apihelpers import primary_key_names
from flask_datatables.views.apihelpers import get_related_model

debug = False

def writedebug(debug, message):
    if debug:
...
class QueryBuilder(object):
    ...
    def _create_operation(model, fieldname, operator, argument, relation=None):
        ...
        * :exc:`AttributeError` if no column with name `fieldname` or
        ...
        writedebug(debug, "Model: {}, Relation: {}, fieldname: {}".format(str(model), str(relation), fieldname))
            ...
            if search_params.order_by:
                for val in search_params.order_by:
                    ...
                    if '__' in field_name:
                        ...
                        query = query.order_by(direction())
            '''
            Since we are only using this file in order to provide
            filtering functionality to datatables,
            we don't need the below else block because datatables is
            handling the order_by on the query
